...
public class PouGameService {
    ...
    @Path("/denuncia")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response denuncia(Denuncia denuncia) throws PouIDNoExisteException {

        Pou pou = jvm.obtenerPou(denuncia.getInformer());
        ...
        if (denuncia.getDate().isEmpty() || denuncia.getInformer().isEmpty() || denuncia.getMessage().isEmpty() ) {
            ...
            return Response.status(404).build(); //PouID que se pasa no existe
        }
